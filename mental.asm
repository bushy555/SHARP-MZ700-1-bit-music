...
;	ld ix,$2121		;enable both tone channels (it is mask to xor with output) +
	ld ix,$0808		;enable both tone channels (it is mask to xor with output) +
...
noMute1
	...
	xor	a
	ld 	h,08
	ld 	de, $0808	;2020 ;$0101 ; $2121	;3333
	exx
	ld 	b,a
drumPtr EQU $+1
	ld 	hl,0
...
noteDivTable
    db #00  ;no div for mute  (MZ-700 adjusted: 245T?293T per iteration)
    
	db #d1,#c5,#ba,#b0,#a6,#9c,#93,#8b
    
	db #83,#7c,#75,#6e,#68,#62,#5d,#58
    
	db #53,#4e,#4a,#45,#41,#3e,#3b,#37
    
	db #34,#31,#2e,#2b,#29,#26,#25,#22
    
	db #21,#1f,#1d,#1c,#1a,#18,#17,#16
    
	db #15,#13,#12,#12,#11,#0f
